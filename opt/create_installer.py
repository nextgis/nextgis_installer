...
def create_installer():
    ...
    if sys.platform == 'darwin':
        ...
        run_shell('security list-keychains -d user -s \"/Users/admin/Library/Keychains/cs1.keychain-db\" \"/Library/Keychains/System.keychain-db\"')
        run_shell('security list-keychains -d user')
        run_shell('security import ./dev.p12 -k \"/Users/admin/Library/Keychains/cs1.keychain-db\" -P \"\" -A')
        run_shell('security set-key-partition-list -S apple-tool:,apple:,codesign: -k {} -s \"/Users/admin/Library/Keychains/cs1.keychain-db\"'.format(args.keychain_password))
        run_shell('security list-keychains')
        run_shell('security list-keychains -d user ')
        run_shell('security find-identity -v -p codesigning')

        run_shell('codesign --deep --force --verify --verbose --keychain \"/Users/admin/Library/Keychains/cs1.keychain-db\" --sign \"{}\" {}'.format(mac_sign_identy, os.path.join(repo_target_path, 'nextgis-setup.app')))
