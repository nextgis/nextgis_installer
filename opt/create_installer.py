...
def parse_arguments():
    ...
    if sys.platform == 'darwin':
        parser.add_argument('-k', dest='keychain_password', required=False, help='Keychain unlock password')
...
def create_installer():
    ...
    if sys.platform == 'darwin':
        ...
        run_shell('security create-keychain -p {} \"/Users/admin/Library/Keychains/cs1.keychain\"'.format(args.keychain_password))
        run_shell('security default-keychain -s \"/Users/admin/Library/Keychains/cs1.keychain\"')
        run_shell('security -v unlock-keychain -p {} \"/Users/admin/Library/Keychains/cs1.keychain\"'.format(args.keychain_password))

        run_shell('security list-keychains -d user -s \"/Users/admin/Library/Keychains/cs1.keychain\" \"/Library/Keychains/System.keychain\"')
        run_shell('security list-keychains -d user')
        run_shell('security import ./dev.p12 -k \"/Users/admin/Library/Keychains/cs1.keychain\" -P \"\" -A')
        run_shell('security set-key-partition-list -S apple-tool:,apple:,codesign: -k {} -s \"/Users/admin/Library/Keychains/cs1.keychain\"'.format(args.keychain_password))
        run_shell('security list-keychains')
        run_shell('security list-keychains -d user ')
        run_shell('security find-identity -v -p codesigning')

        run_shell('codesign --deep --force --verify --verbose --keychain \"/Users/admin/Library/Keychains/cs1.keychain\" --sign \"{}\" {}'.format(mac_sign_identy, os.path.join(repo_target_path, 'nextgis-setup.app')))
