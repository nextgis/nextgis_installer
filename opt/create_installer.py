...
def create_installer():
    ...
    if sys.platform == 'darwin':
        ...
        run_shell('security unlock-keychain -p {} System.keychain && security import ./dev.p12 -k System.keychain -P \"\" -A && security set-key-partition-list -S apple-tool:,apple:,codesign: -k {} -s System.keychain && security find-identity -v -p codesigning && codesign --deep --force --verify --verbose --sign \"{}\" {}'.format(args.keychain_password, args.keychain_password, mac_sign_identy, os.path.join(repo_target_path, 'nextgis-setup.app')))
